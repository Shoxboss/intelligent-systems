import { parseMsg, IMessage } from "../msg";
import { Socket } from "dgram";
import { IPlayerState } from "../interfaces";

import DecisionTreeManager from "../helpers/BT/manager";

import { ICommand } from "../helpers/interfaces";
import { PassDT } from "../helpers/BT/make_pass";
import { GoalDT } from "../helpers/BT/make_goal";
...
export default class Agent {
  ...
  analyzeEnv(data: IMessage) {
    ...
    const _p = p as unknown as string[];

    if (cmd === "hear" && _p[2].includes("kick_off_")) {
      PassDT.state.isGoal = false;
      GoalDT.state.isGoal = false;
      PassDT.state.next = 0;
      GoalDT.state.next = 0;
    }
    if (cmd === "hear" && _p[2].includes("goal_l_")) {
